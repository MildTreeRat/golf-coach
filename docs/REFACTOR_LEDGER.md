...
| 2026-08-15 | R4 | `mcp/server.py` tool descriptions → `contracts/tool_descriptions.py` | Done | ADR-020 added a second adapter over the same query functions (the in-process tool runner), and a model picks a tool by reading its description and nothing else. Leaving the prose inline would have made eight load-bearing paragraphs exist twice — the failure this repo has been bitten by three times, and the one `contracts/caveats.py` already exists to prevent. Moved verbatim (verified byte-for-byte against HEAD), and the move made them scannable on a base install, so `tests/test_docs_truth.py`'s count-claim scan now covers the tool descriptions as well as the output schemas. |
