...
22 decisions, 22 addenda between them (`grep -c '^#\+ *Addendum' docs/decisions/*.md` — the
...
| [022](decisions/022-learned-artifacts-as-committed-data.md) | Learned artifacts as committed data — and the tour joint-distribution model | Accepted, **both models surfaced** | **1** — a second artifact (the trajectory model) under the same rule; `z` lost its A/B, the anchor set nearly made it unusable, and a pixel-aspect bug was worth 7 points of variance |
